...
def generate_embeddings(name, data, encoder, transform, save_root, regenerate=False):
    """
        ...
        Args:
            dataset <json>
                JSON styled object containing image metadata
...
class NeighborsDataset(Dataset):
    ...
    def __init__(self, name, embedding_net, augment_fun, cutout_fun, transforms, n_neighbors):

        self.dataset = datasets[name](root='../saved_data/datasets/'+name, train=True, transform=None, download=True)
        self.embeddings = generate_embeddings(name, self.dataset, embedding_net, 
            transforms['standard'], save_root='../saved_data/other/', regenerate=False) 
        self.neighbor_indices = find_neighbors(self.embeddings, k=n_neighbors)
        self.augment_fun = augment_fun
        self.cutout_fun = cutout_fun

        if isinstance(transforms, dict):
            self.image_transform = transforms['standard']
            self.augment_transform = transforms['augment']
        else:
            raise TypeError('Transformation object should be a dictionary')
    ...
    def __getitem__(self, idx):
        ...
        # Perform cutout 10% of the time
        if random.random() < 0.1:
            return_obj['neighbor'] = self.cutout_fun(return_obj['neighbor'])
...
def generate_data_loaders(name, batch_size, n_neighbors, transforms, embedding_net, augment_fun, cutout_fun):
    """
    ...
    train_dset = NeighborsDataset(name, embedding_net, augment_fun, cutout_fun, transforms, n_neighbors)
