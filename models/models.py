...
class RotNet:
    ...
    def __init__(self, config, device, output_dir):
        ...
        self.encoder = networks.Encoder(**config['encoder']).to(self.device)
        common.print_network(self.encoder, 'Encoder')
        self.head = networks.RotnetClassifier(in_dim=self.encoder.backbone_dim).to(self.device)
        common.print_network(self.head, 'Classification Head')
    ...
    def load_ckpt(self):
        ckpt = torch.load(os.path.join(self.output_dir, 'last.ckpt'))
        self.encoder.load_state_dict(ckpt['enc'])
        self.proj_head.load_state_dict(ckpt['head'])
        self.optim.load_state_dict(ckpt['optim'])
        if self.lr_scheduler is not None:
            self.lr_scheduler.load_state_dict(ckpt['lr_scheduler'])
        return ckpt['epoch']
    ...
    def knn_mining_eval(self, loader, topk=20):

        fvecs, labels = [], []

        for batch in loader:
            img, trg = batch['img'].to(self.device), batch['target']
            with torch.no_grad():
        ...
        wandb.log({'KNN match accuracy': accuracy})

        if accuracy > self.best:
            self.best = accuracy
            self.save_model(f"best_rotnet_encoder_{self.config['dataset']}_{self.config['encoder']['name']}.ckpt")

        return accuracy


    def rotation_prediction_eval(self, loader):

        total_correct = 0
        with torch.no_grad():
            for batch in loader:
                img, labels = batch['img'].to(self.device), batch['target'].to(self.device)
                preds = self.head(self.encoder(img)).argmax(dim=-1)   
                total_correct += preds.eq(labels.view_as(preds)).sum().item() 
                pbar.update(1)
        
        acc = total_correct/len(loader.dataset) * 0.25
        wandb.log({'Rotation prediction accuracy': acc})
    ...
    def train_one_step(self, data):
        ...
        out = self.head(self.encoder(img))
    ...
    def validate(self, val_loader):
        ''' KNN matching and rotation prediction accuracy '''

        knn_acc = self.knn_mining_eval(val_loader, topk=20)
        rot_pred_acc = self.rotation_prediction_eval(val_loader)
        return {'knn_acc': knn_acc, 'rotation_pred_acc': rot_pred_acc}
