...
class SimCLR:
    def __init__(self, config, device, output_dir):
        ...
        self.optim = train_utils.get_optimizer(
            config=config["simclr_optim"], params=list(self.encoder.parameters()) + list(self.proj_head.parameters())
        )
        self.lr_scheduler, self.warmup_epochs = train_utils.get_scheduler(
            config={**config["simclr_lr_scheduler"], "epochs": config["epochs"]}, optimizer=self.optim
        )
    ...
    def save_model(self, fname):
        model_save = {"enc": self.encoder.state_dict()}
    ...
    def linear_eval(self, train_loader, val_loader):
        ...
        clf_head = networks.ClassificationHead(
            in_dim=self.encoder.backbone_dim, n_classes=DATASET_HELPER[self.config["dataset"]]["classes"]
        ).to(self.device)
        clf_optim = train_utils.get_optimizer(config={**self.config["clf_optim"]}, params=clf_head.parameters())
        clf_scheduler, _ = train_utils.get_scheduler(
            config={**self.config["clf_lr_scheduler"], "epochs": self.config["linear_eval_epochs"]}, optimizer=clf_optim
        )
...
class RotNet:
    def __init__(self, config, device, output_dir):
        ...
        self.optim = train_utils.get_optimizer(
            config=config["rotnet_optim"], params=list(self.encoder.parameters()) + list(self.cls_head.parameters())
        )
        self.lr_scheduler, self.warmup_epochs = train_utils.get_scheduler(
            config={**config["rotnet_lr_scheduler"], "epochs": config["epochs"]}, optimizer=self.optim
        )
...
class ClusteringModel:
    def __init__(self, config, device, output_dir):
        ...
        self.lr_scheduler, self.warmup_epochs = train_utils.get_scheduler(
            config={**config["cluster_lr_scheduler"], "epochs": config["epochs"]}, optimizer=self.optim
        )
    ...
    def save_model(self, fname):
        ...
        model_save = {"enc": self.encoder.state_dict(), "cluster_head": dict(zip(w_keys, w))}
        torch.save(model_save, os.path.join(self.output_dir, fname))
...
class SelfLabel:
    def __init__(self, config, device, output_dir):
        ...
        self.lr_scheduler, self.warmup_epochs = train_utils.get_scheduler(
            config={**config["selflabel_lr_scheduler"], "epochs": config["epochs"]}, optimizer=self.optim
        )
    ...
    def save_model(self, fname):
        model_save = {"enc": self.encoder.state_dict(), "cluster_head": self.cluster_head.state_dict()}
        torch.save(model_save, os.path.join(self.output_dir, fname))
