...
# Multiprocessing fix
# In case "too many open files" error occurs
...
class Trainer:
    ...
    def __init__(self, args):
        ...
        for loader_type, params in self.config['dataloaders'].items():
            ...
            if params.get('rotnet', False):
                dataset = datasets.RotnetDataset(self.config, loader_type, transforms)
            
            shuffle = params.get('shuffle', False)
            weigh = params.get('weigh', False)
            drop_last = params.get('drop_last', False)
            collate = params.get('collate', False)

            self.data_loaders[loader_type] = datasets.get_dataloader(
                dataset=dataset, 
                ...
                weigh=weigh,
                drop_last=drop_last,
                collate_fn=train_utils.rotnet_collate_fn if collate else None
            )
        
        assert self.config['task'] in MODEL_HELPER.keys(), 'Invalid task, choose from (simclr, rotnet, cluster, selflabel)'
